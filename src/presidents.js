...
function getNames(presidentsArr) {
  return presidentsArr.map((x) => x.name);
}

// Iteration 2 | Democratic Presidents - `filter()`
function getDemocraticPresidents(presidentsArr) {
  return presidentsArr.filter((x) => x.party === "Democratic");
}

// Iteration 3 | Count Years in Office - reduce()
function countYearsInOffice(presidentsArr) {
  return presidentsArr.reduce((pV, cV) => {
    if (cV.leftOffice === null) return pV;
    pV += cV.leftOffice - cV.tookOffice;
    return pV;
  }, 0);
}
...
function sortPresidentsByBirthYear(presidentsArr) {
  return presidentsArr.sort(({ birthYear: a }, { birthYear: b }) => a - b);
}
...
function countRepublicanPresidents(presidentsArr) {
  return presidentsArr.reduce((pV, cV) => {
    if (cV.party !== "Republican") return pV;
    pV += 1;
    return pV;
  }, 0);
}
...
function sortPresidentsByName(presidentsArr) {
  return presidentsArr.sort(({ name: a }, { name: b }) => a.localeCompare(b));
}
